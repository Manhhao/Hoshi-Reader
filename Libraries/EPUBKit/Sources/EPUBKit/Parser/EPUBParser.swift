...
public final class EPUBParser: EPUBParserProtocol {
    ...
    public func parse(documentAt path: URL) throws -> EPUBDocument {
        ...
        do {
            ...
            // Note Manhhao: This was changed to support .epub files without a ncx file
            ...
            let tocId = spine.toc ?? manifest.items.first(where: { $0.value.mediaType == .opf2 })?.key
            ...
            guard let toc = tocId, let fileName = manifest.items[toc]?.path else {
...
extension EPUBParser: EPUBParsable {
    ...
    public func getSpine(from xmlElement: XMLElement, manifestItems: [EPUBManifestItem]) -> EPUBSpine {
        // Pass the manifest items to the updated parser
        spineParser.parse(xmlElement, manifestItems: manifestItems)
    }
